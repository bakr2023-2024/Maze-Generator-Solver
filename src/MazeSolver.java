...
public class MazeSolver {
    ...
    public static volatile int delay;
    ...
    private boolean wallcollide(int sx, int sy) {
        if ((sx >= 0 && sx < this.map.length) && (sy >= 0 && sy < this.map[0].length)
                && (this.map[sx][sy] != 1) && (this.map[sx][sy] != -6))
            return false;
        return true;
    }
...
/*
 * Djikstra
 * HashMap<Vertex, ArrayList<Vertex>> list = new HashMap<Vertex,
 * ArrayList<Vertex>>();
 * HashMap<Vertex, Integer> table = new HashMap<Vertex, Integer>();
 * HashSet<Vertex> unvisited = new HashSet<Vertex>();
 * private void addNeighbor(int r, int c, int y, int x) {
 * Vertex k = new Vertex(r, c, null);
 * Vertex neighbor = new Vertex(r + y, c + x, null);
 * if (this.unvisited.contains(neighbor))
 * return;
 * this.list.get(k).add(neighbor);
 * this.list.get(neighbor).add(k);
 * this.table.put(k, Integer.MAX_VALUE);
 ...
 * if (!collide(i, j))
 * this.list.put(new Vertex(i, j, null), new ArrayList<Vertex>());
 * }
 * for (int i = 0; i < this.map.length; i++) {
 * for (int j = 0; j < this.map[0].length; j++) {
 * if (collide(i, j))
 ...
 * addNeighbor(i, j, 0, -1);
 * }
 * this.unvisited.add(new Vertex(i, j, null));
 * }
 * }
 * }
 * 
 ...
 * return Integer.compare(table.get(v1), table.get(v2));
 * }
 * });
 * 
 * private Vertex djikstra(Vertex curr) {
 * while (!pq.isEmpty()) {
 * if (!keepSolving)
 * return null;
 * curr = pq.poll();
 ...
 * return curr;
 * }
 * if (!this.unvisited.contains(curr))
 * continue;
 * this.map[curr.x][curr.y] = -1;
 * for (Vertex neighbor : this.list.get(curr)) {
 * if (this.table.get(curr) + 1 < this.table.get(neighbor)) {
 * this.table.put(neighbor, this.table.get(curr) + 1);
 * }
 * neighbor.prev = curr;
 * pq.add(neighbor);
 * }
...
/*
 * private void miniDFS(Vertex curr) {
 * Vertex x;
 * if (cellType(curr) == 0) {
 * this.map[curr.x][curr.y] = 1;
 * return;
 ...
 * }
 * 
 * private boolean notStraight(Vertex curr) {
 * if (cellType(curr) != 1)
 * return false;
 * boolean top = false;
 * boolean right = false;
 ...
 * return true;
 * }
 * private Vertex culdesac(Vertex curr) {
 * for (int i = 1; i < this.map.length - 1; i++) {
 * for (int j = 1; j < this.map[0].length - 1; j++) {
 * if (!wallcollide(i, j) && notStraight(new Vertex(i, j, null))) {
 * miniDFS(new Vertex(i, j, null));
 * }
 * }
 * }
 * return deadend(curr);
